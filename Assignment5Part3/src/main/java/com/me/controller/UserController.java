...
public class UserController {
	...
	@RequestMapping(value = "/user/loginsecond", method = RequestMethod.POST)
	protected String goToUserHome1(HttpServletRequest request) throws Exception {
		if(request.getParameter("username").equals("testuser") && request.getParameter("password").equals("1234")) {
			return "index";
		}else {
	...
	protected ModelAndView registerNewUser(HttpServletRequest request,  @ModelAttribute("user") User user, BindingResult result) throws Exception {
		...
		try {
			...
			return new ModelAndView("movie", "user", u);
